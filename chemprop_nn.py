...
class AttentionMechanism(nn.Module):
    def __init__(self, in_features):
        ...
        self.W = nn.Linear(in_features, in_features)  # Parameter matrix for computing attention scores

    def forward(self, x):
        # x: node features (N x in_features)
        
        # Compute attention scores
        att_scores = torch.matmul(x, self.W(x).transpose(0, 1))  # Dot product attention
        
        # Softmax
        att_weights = torch.nn.functional.softmax(att_scores, dim=1)
        
        # Apply attention
        attended_features = torch.matmul(att_weights, x)
...
class MPNEncoder(nn.Module):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int):
        ...
        self.attn_atom = AttentionMechanism(atom_fdim)
        self.attn_bond = AttentionMechanism(bond_fdim)
    ...
    def forward(self,
        ...
        for depth in range(self.depth - 1):
            ...
            if self.atom_messages:
                attended_atom_features = self.attn_atom(f_atoms)

                nei_a_message = index_select_ND(message, a2a)  # num_atoms x max_num_bonds x hidden
                nei_f_bonds = index_select_ND(f_bonds, a2b)  # num_atoms x max_num_bonds x bond_fdim
                nei_message = torch.cat((nei_a_message, nei_f_bonds), dim=2)  # num_atoms x max_num_bonds x hidden + bond_fdim
                
                nei_message = torch.cat((attended_atom_features.unsqueeze(1).repeat(1, nei_a_message.size(1), 1), nei_a_message), dim=2)
                
                message = nei_message.sum(dim=1)  # num_atoms x hidden + bond_fdim
            else:
                attended_bond_features = self.attn_bond(f_bonds)
                ...
                attended_bond_features = attended_bond_features.unsqueeze(0)
                attended_bond_features = attended_bond_features.expand(a_message.size(0), -1, -1)  # num_atoms x num_bonds x hidden
